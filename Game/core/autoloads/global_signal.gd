...
func emit_signal_when_ready(signal_name: String, args: Array, emitter: Object) -> void:
	if not _emitters.has(signal_name):
		AppLogger.error(LOG_MODULE, "GlobalSignal.emit_signal_when_ready: signal '%s' is not registered." % signal_name)
		return
...
func _process_purge(data: Dictionary, group: Dictionary) -> bool:
	...
	if not object_exists or data.object.get_instance_id() != data.object_id:
